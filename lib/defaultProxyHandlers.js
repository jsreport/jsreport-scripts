...
async function documentStoreFind (reporter, originalReq, spec) {
  ...
  const res = await reporter.documentStore.collection(spec.data.collection).find(spec.data.query, localReq)
  return res
}
...
async function documentStoreFindOne (reporter, originalReq, spec) {
  const localReq = reporter.Request(originalReq)
  localReq.context.userFindCall = true
  const res = await reporter.documentStore.collection(spec.data.collection).findOne(spec.data.query, localReq)
  return res
}
