...
module.exports = function (inputs, callback, done) {
  ...
  const doneWrap = function (err) {
      ...
      request: {
        ...
        context: sandboxContext.__request.context
      },
  ...
  const sandboxEnv = safeSandbox(
    initialSandbox,
    {
      ...
      formatError: (error, moduleName) => {
        error.message += error.message += ` To be able to require custom modules you need to add to configuration { "renderingSource": "trusted" } or enable just specific module using { "extensions": { "scripts": { "allowedModules": ["${moduleName}"] } }`
      },
