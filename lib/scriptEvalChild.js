...
module.exports = function (inputs, callback, done) {
  ...
  const doneWrap = function (rawErr) {
    ...
    if (rawErr != null) {
      ...
      if (!isValidError) {
        err = new Error('Script is trying to use value as an error but value is not valid error, please check script content and use valid error instance or string when throwing errors')
      } else {
