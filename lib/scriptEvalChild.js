...
module.exports = function (inputs, callback, done) {
    ...
    var doneWrap = function (err) {
        Object.keys().forEach(function (k) {
            if (k !== 'reporter') {
                input.request[k] = backCompatibleRequest[k]
            }
        })
    ...
    Object.keys(input.request).forEach(function (k) {
        backCompatibleRequest[k] = input.request[k]
    })
    ...
    var runBeforeRender = "\nif (typeof beforeRender === 'function') { beforeRender(request, response, done); } else { if (typeof afterRender === 'function') doneMethods(); }"
    var runAfterRender = "\nif (typeof afterRender === 'function') { afterRender(request, response, done); } else { done(); }"
