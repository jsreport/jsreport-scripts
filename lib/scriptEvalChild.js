...
module.exports = function (inputs, callback, done) {
  ...
  const runBeforeRender = "\n__request.shouldRunAfterRender = typeof afterRender === 'function'; if (typeof beforeRender === 'function') { beforeRender(__request, __response, __done); } else {  if (typeof afterRender === 'function') __done()  }"
  const runAfterRender = "\nif (typeof afterRender === 'function') { afterRender(__request, __response, __done); } else { __done(); }"
