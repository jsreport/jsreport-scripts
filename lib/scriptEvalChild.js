...
module.exports = async function scriptEvalChild (inputs, callbackProxyHandleAsync) {
  ...
  const jsreportProxy = createJsreportProxy({
    ...
    beforeMethodExecute: (input) => {
      // when using in-process we want the proxy because it ensure some typeof/constructor checks
      // to be right when going between script/callback execution
      if (inputs.strategy === 'in-process') {
        return
      }

      // when using strategy that involves some kind of serialization between master/process
      // we need to modify the input data for the callback, we need to remove proxies
      // before executing callback because when using worker threads if you pass a proxy
      // that its going to be send to master/worker it throw a DataClone error when sending
      // the message, and we want to avoid that.
      for (const [key, value] of Object.entries(input.data)) {
        input.data[key] = unproxyValue(value)
      }
    },
  ...
  const getScriptResult = function (rawErr) {
    ...
    return {
      ...
      request: {
        ...
        data: err == null ? newData : undefined,
  ...
  const unproxyValue = sandboxEnv.unproxyValue
