...
class Scripts {
  ...
  async handleBeforeRender (request, response) {
    ...
    return Promise.mapSeries(scripts, (script) => this._handleOneBeforeRender(request, response, script))
  }

  handleAfterRender (request, response) {
    return Promise.mapSeries(request.context._parsedScripts, (script) => this._handleOneAfterRender(request, response, script))
  }

  async _handleOneBeforeRender (request, response, script) {
  ...
  async _handleOneAfterRender (request, response, script) {
    const body = await this.scriptManagerExecute({
      script: script,
      allowedModules: this.definition.options.allowedModules,
      appDirectory: this.reporter.options.appDirectory,
      rootDirectory: this.reporter.options.rootDirectory,
  ...
  async _handleCallback (originalReq, req, cb) {
    ...
    try {
      const res = await this.reporter.render(req)
