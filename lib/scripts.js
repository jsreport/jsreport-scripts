...
class Scripts {
  constructor (reporter, definition) {
    ...
    if (this.reporter.execution) {
      this.addProxyMethods(this.reporter.execution.resolve('defaultProxyMethods.js'))
    } else {
      this.addProxyMethods(path.join(__dirname, 'defaultProxyMethods.js'))
    }

    this.addProxyHandlers(defaultProxyHandlers)
  }

  addProxyMethods (modulePath) {
    this._proxyMethods.push(modulePath)
  }

  addProxyHandlers (handlerCreator) {
    this._proxyHandlers.push(handlerCreator)
  }
