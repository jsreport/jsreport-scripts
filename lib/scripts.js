...
class Scripts {
  constructor (reporter, definition) {
    ...
    this.addProxyMethods(path.join(__dirname, 'defaultProxyMethods.js'), path.join(__dirname, 'defaultProxyHandlers.js'))
  }

  addProxyMethods (modulePath, moduleHandlerPath) {
    this._proxyMethods.push(modulePath)
    this._proxyHandlers.push(moduleHandlerPath)
  }
  ...
  async _executeScript (request, response, script, method) {
    this.reporter.logger.debug(`Executing script ${(script.name || script.shortid || 'anonymous')} using ${this.reporter.options.templatingEngines.strategy} strategy`, request)
    ...
    const scriptDef = {
      ...
      proxyHandlers: this._proxyHandlers,
      method: method,
    ...
    const body = await this.reporter.executeScript(scriptDef, {
      ...
      callbackModulePath: path.join(__dirname, 'scriptCallbackProxyHandle.js')
    }, request)
